...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (!postId || postId === "" || !title || title === "" || !content || content === "") return NextResponse.json({ success: false, error: "Post id, title and content are required!" }, { status: 400 });
        ...
        if (isallowed.userRole !== UserRole.ADMIN) {
            ...
            if (currentPost.userId !== isallowed.userId) return NextResponse.json({ success: false, error: "Unauthorized!" }, { status: 401 });
        }
